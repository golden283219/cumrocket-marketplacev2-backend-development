...
build {
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      # Build Dockers
      "cd ${local.backend_path}",
      "sudo docker-compose -f docker-compose.api.yml pull",
      "sudo docker-compose -f docker-compose.api.yml build",

      # Run on reboot
      "(sudo crontab -l 2> /dev/null ; echo '@reboot (sleep 30s ; cd ${local.backend_path} ; /usr/local/bin/docker-compose -f docker-compose.api.yml up -d )&' ) | sudo crontab -",
    ]
  }
}
